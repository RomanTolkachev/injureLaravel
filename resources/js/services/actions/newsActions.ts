import { getFourNews, getNewsItem } from "../utils/api";
import { AppThunk, INewsActions } from "../utils/types";
import { INews } from "../utils/newsType";

export const SET_NEWS_REQUEST_SENT: "SET_NEWS_REQUEST_SENT" =
    "SET_NEWS_REQUEST_SENT";
export const SET_NEWS_REQUEST_SUCCESS: "SET_NEWS_REQUEST_SENT" =
    "SET_NEWS_REQUEST_SENT";
export const SET_NEWS_REQUEST_ERROR: "SET_NEWS_REQUEST_ERROR" =
    "SET_NEWS_REQUEST_ERROR";
export const PUSH_NEWS: "PUSH_NEWS" = "PUSH_NEWS";
export const SET_NEWS_REQUEST_OVER: "SET_NEWS_REQUEST_OVER" =
...
export const setNewsRequestFailed = (errorMessage: string): INewsActions => {
    return {
...
export const requestNews = (page: number): AppThunk => {
    return (dispatch) => {
        ...
        getFourNews(page)
            ...
            .catch((err) => dispatch(setNewsRequestFailed(err.error)))
            .finally(() => dispatch(setNewsRequestOver()));
    };
};

export const requestNewsItem = (uuid: string): AppThunk => {
    return (dispatch) => {
        dispatch(setNewsRequestSent());
        getNewsItem(uuid)
            .then((res: any) => dispatch(pushNews(res)))
            .catch((err) => {
                dispatch(setNewsRequestFailed(err.error));
            })
            .finally(() => dispatch(setNewsRequestOver()));
    };
};
